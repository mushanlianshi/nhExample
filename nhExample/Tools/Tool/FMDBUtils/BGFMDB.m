...
-(void)executeSQL:(NSString *)SQL complete:(void (^)(BOOL isSuccess))complete{
    __block BOOL result;
    [self.queue inDatabase:^(FMDatabase *db) {
...
-(void)createTableWithTableName:(NSString*)name keys:(NSArray*)keys complete:(void (^)(BOOL isSuccess))complete{
    [self createTableWithTableName:name keys:keys primaryKey:nil complete:complete];
//    if (name == nil) {
//        NSLog(@"表名不能为空!");
//        if (complete) {
//            complete(NO);
...
-(void)createTableWithTableName:(NSString*)name keys:(NSArray*)keys primaryKey:(NSString *)primaryKey complete:(void (^)(BOOL isSuccess))complete{
    if (name == nil) {
        NSLog(@"表名不能为空!");
        if (complete) {
            complete(NO);
    ...
    [self.queue inDatabase:^(FMDatabase *db) {
        ...
        if(primaryKey && primaryKey.length>0){
            header = [NSString stringWithFormat:@"create table if not exists %@ (%@ integer primary key autoincrement",name,primaryKey];
        }else{
            header = [NSString stringWithFormat:@"create table if not exists %@ (id integer primary key autoincrement",name];
        }
//        NSString* header = [NSString stringWithFormat:@"create table if not exists %@ (id integer primary key autoincrement",name];//,name text,age integer);
        NSMutableString* sql = [[NSMutableString alloc] init];
        [sql appendString:header];
